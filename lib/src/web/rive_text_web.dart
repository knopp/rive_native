...
class RawPathWasm extends RawPath {
  ...
  RawPathWasm(this.pointer, {required this.verbs, required this.points});
...
class RawPathCommandWasm extends RawPathCommand {
  ...
  RawPathCommandWasm._(super.verb, this._points, this._pointsOffset);
...
class TextShapeResultWasm extends TextShapeResult {
  static final _finalizer = Finalizer<js.JSAny>(
    (ptr) => _deleteShapeResult.callAsFunction(null, ptr),
  );
  int _shapeResultPtr;
  ...
  BreakLinesResult breakLines(
    ...
    _deleteLines.callAsFunction(null, rawResult.toJS);
...
extension ByteDataWasm on ByteData {
  WasmDynamicArray readDynamicArray(int offset) {
    ...
    return WasmDynamicArray(ptr, size);
  }
...
class LinesWasm extends ListBase<GlyphLineWasm> {
  ...
  GlyphLineWasm operator [](int index) {
    ...
    return GlyphLineWasm(
      RiveWasm.heapDataView(wasmDynamicArray.ptr + index * lineSize),
    );
  }
...
class FontWasm extends Font {
  ...
  TextShapeResult computeShape(
    ...
    var writer = BinaryWriter(alignment: runs.length * sizeOfNativeTextRun);
  ...
  Iterable<FontTag> get features {
    ...
    return features.map(
      (value) => FontTagWasm((value as js.JSNumber).toDartInt),
    );
  }
...
void disableFallbackFonts() {
  _disableFallbackFonts.callAsFunction(null);
}

void enableFallbackFonts() {
  _enableFallbackFonts.callAsFunction(null);
}

class TextHelperWeb extends TextHelper {
  @override
  double convertStringToNumber(String value) =>
      RiveWasm.toNativeString(value, (valuePointer) {
